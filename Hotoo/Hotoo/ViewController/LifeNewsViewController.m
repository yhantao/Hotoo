...
@interface LifeNewsViewController ()
...
@implementation LifeNewsViewController
...
- (void)viewDidLoad {
    ...
    // add scroll view
    UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 142, self.view.frame.size.width, self.view.frame.size.height - 140)];
...
- (HTNewsViewController *)addNewsControllerIfNotPresent:(NSInteger)index{
    if (!self.map[@(index)]){
        ...
        HTNewsViewController *vc = [[HTNewsViewController alloc] initWithIndex:(HTNewsType)(5+index)];
...
- (NSArray *)lblTexts{
    if (!_lblTexts){
        _lblTexts = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"LifeNewsURLs" ofType:@"plist"]];
    }
    return _lblTexts;
}
